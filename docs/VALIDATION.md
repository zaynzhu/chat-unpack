...
- 用户真实深色截图实测（2026-08-31）：无预处理时整页乱码碎片（Windows OCR 对深底浅字识别失效）。已修复：`ImportedImage` 加入 OCR 预处理——2x 放大（对齐捕获路径）+ 亮度采样自动反色（深色底 < 0.45 阈值时）；深色合成图回归通过（9/9，OCR 行文本完全干净），超大图（>400 万像素）跳过放大防爆内存。
