...
>
> 开发中平台：Windows 11 23H2+、x64；真实微信 4.x 防截屏（WDA）实测封锁捕获路线后（见 2.4），已于 2026-08-29 转向「截图导入」模式并完成实现与验证（见 2.5）；捕获路线仅保留 Fixture 调试
>
> 状态日期：2026-08-29
...
待决策方向（命中停止条件，回到设计）→ **2026-08-29 已决策：转向截图导入模式**（用户选定；旧版微信因强制升级风险不采用，捕获路线暂停）：

- ~~评估微信 3.9.x（计划原定不兼容）是否无 WDA 保护~~ 不采用：即使可行也是死胡同，产品被钉在随时失效的旧版上。
- **已采用：产品转向「导入截图」**，见 2.5。
- 或暂停 Windows 端捕获路线，保留 macOS 路线（macOS 无 display affinity 机制，ScreenCaptureKit 不受影响）——继续适用：macOS 版不受本次结论影响。

仍未验证（不记为已通过）：

- 修复后 WGC 路径在应用内经 FixtureHost 完整回归。
- ~~上述任一方向决策后的新路线验收~~ → 已按 2.5 完成截图导入管线的合成数据端到端 + 实机 UI 冒烟；真实微信截图的识别质量仍待用户自行试用校准。

### 2.5 Windows 截图导入模式（已实现并验证）

2026-08-29 用户选定截图导入方向后实现。产品形态：用户用微信自带截图（Alt+A，进程内不受 WDA 限制）分屏截取合并记录，贴入应用，本地 OCR → 解析 → 跨图拼接 → Markdown → 1800 字符分段复制。自动扫描入口仅剩 Fixture 调试模式可用（Release 禁用，按钮显示禁用原因）。

实现（提交 3c22753、8685219）：

- `Import/ImportRecognitionService`：图片 → `WindowsOCRService`（全图区域）→ `MessageParser` → `TranscriptAssembler`（视口号 = 截图序号），完全复用扫描流程的 Core 管线；相邻截图重叠由 OverlapMatcher 去重，用户无需精确对齐。
- `Import/ImportedImage`：剪贴板/文件统一转冻结 Bgra32 预览 + SoftwareBitmap；图片只在内存，识别完成或清除时 Dispose。
- UI：新增 `Importing` 状态与导入页——Ctrl+V 粘贴（窗口级 KeyBinding，CanExecute 门控不干扰结果页文本框）、图片文件拖拽、缩略图队列（逐张移除/清空）、识别进度、返回待机。修正了误导性文案（待机页/结果页不再标"Fake 数据"；顶部常驻"完全离线 · 本地处理"）。

验证证据：

- 合成截图端到端：GDI 绘制 3 张模拟微信布局的虚构聊天截图（9 条消息、图 2/3 与前图重叠 1 条），经真实 Windows OCR + Core 管线输出 **9 条消息、状态完整、跨图去重正确**；OCR 丢失昵称的块正确回退"未知发言人"，日期前缀正确进正文不被当成发言人。
- 隐私静态扫描：Import 新代码无网络/数据库/进程读写/SendInput 命中。
- 实机 UI 冒烟（真实 Windows 11）：启动 → 待机页双入口（导入可用、自动扫描按预期禁用）→ 导入页 → 粘贴剪贴板图片（缩略图/计数正确）→ 开始识别 → 结果页"状态：完整 · 消息：4"，标题"检查结果"，Markdown 渲染正确，保存/复制按钮激活。
- Core 回归：136 项检查通过（含新增截图导入计数改动前基线）。

已知限制（截图导入模式）：

- 真实微信截图的 OCR 质量（昵称漂移、表情/图片占位）尚未用真实记录校准；结果页可手动编辑兜底。
- 合成测试中发现 Windows OCR 对紧凑行距的时间框定位偏差可达约 30px（真实微信行距更大，FixtureHost 端到端 257 条证明真实布局无此问题）；若真实截图出现发言人/正文错位，优先加大截图行距（微信默认即满足）。
- 拖拽导入仅接受 png/jpg/jpeg/bmp；不识别的文件计数跳过并提示。
- 剪贴板 DIB alpha 修复（2026-08-31 用户实测 0 条 → 链路全通）：微信 Alt+A 剪贴板图的 alpha 字节为脏数据，WPF 预乘解读后整图透明，OCR 零行。`ToOcrSoftwareBitmap` 送 OCR 前强制 alpha=255（CopyPixels 原始 Bgra32 的 RGB 未预乘，安全）；零内容诊断（.scandiag.log：尺寸/行数/结果状态）证实同图剪贴板路径 0 行 → 15 行。
- 合并记录窗首个真实样例（2026-08-31 用户实测）：4/4 条逐条切分成功、OWenY 发言人正确、日期泄漏与 12:09→02:09 错位由 TimestampParser 数字空格规整（"1 2:09"→"12:09"）与纯日期前缀不回插正文修复；单字短昵称（"城"）OCR 漏检时按设计回退"未知发言人"，结果页可手补。
- 布局差异确认：普通聊天窗口同一分钟只有一条居中时间分隔（气泡无独立时间列），解析为"该时间段 1 条聚合消息 + 未知发言人"——符合保守设计（有行无锚不猜边界）；「合并聊天记录」窗口每条消息头有"昵称+时间"同行，可逐条切分，是本工具的目标布局。
- 用户真实深色截图实测（2026-08-31）：无预处理时整页乱码碎片（Windows OCR 对深底浅字识别失效）。已修复：`ImportedImage` 加入 OCR 预处理——2x 双三次插值放大（TransformedBitmap HighQuality，超大图 >400 万像素跳过防爆内存）+ 亮度采样自动反色（深色底 < 0.45 阈值）+ 温和灰阶拉伸（动态范围 <64 才触发，1% 分位截断）。调参过程：5%~95% 强拉伸抹掉笔画细节反而更差；逐像素放大无插值效果差；3x 过强同样丢字。深色合成图 9/9 通过；真实深色截图修复前后对比：时间"0 圭 52"→"09:52"、正文"收 至 上 成 哥 牛"→"收 到 , 成 哥 牛 逼"、标题括号补全，"傻"等生僻组合字仍误认（系统 OCR 字典限制，结果页可手改）。修复期间发现并修复整图同色时 255/0 除零崩溃（0xC0000094）。
